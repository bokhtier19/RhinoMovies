...
const MovieCard: React.FC<CardProps> = ({ id, title, name, poster_path, release_date, first_air_date, type, media_type }) => {
    const displayTitle = title || name || "Untitled";
    const displayDate = release_date || first_air_date || "N/A";

    // Decide type: priority -> media_type -> prop type -> fallback
    const displayType = media_type === "movie" ? "MOVIE" : media_type === "tv" ? "TV" : type || (title ? "MOVIE" : "TV");
