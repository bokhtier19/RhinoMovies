...
import {TVShow} from "@/src/app/interfaces/interface";
...
const TVShowCard: React.FC<TVShowCardProps> = ({id, name, poster_path, first_air_date, number_of_seasons, number_of_episodes}) => {
    return (
        <Link href={`/shows/${id}`}>
            <div className="w-44 flex-shrink-0 bg-imdb-black rounded-sm overflow-hidden hover:scale-105 transition-transform">
                <div className="relative w-full h-64">
                    <Image src={poster_path ? `https://image.tmdb.org/t/p/w500${poster_path}` : "/no-image-available.png"} alt={name} fill style={{objectFit: "cover"}} />
                </div>
                <div className="p-2">
